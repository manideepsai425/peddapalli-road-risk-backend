...
main.py
=======
Peddapalli Road Risk — FastAPI Backend  v1.1

Startup:
    uvicorn main:app --host 0.0.0.0 --port 8000
    (Render will run: uvicorn main:app --host 0.0.0.0 --port $PORT)

Place accidents.csv at:
    <project_root>/data/accidents.csv
"""
...
# Instantiated at module level; train() is called inside lifespan so the
# model is ready before the first request is served.
...
async def lifespan(app: FastAPI):
    ...
    model_loader.train()
    logger.info(
        "  Network: %d segments | %d edges",
        len(model_loader.SEGMENT_DATA),
        len(model_loader.EDGE_LIST),
    )
...
def predict_route(req: RouteRequest):
    ...
    try:
        ...
        logger.error("Routing error: %s", exc, exc_info=True)
...
def get_heatmap(weather: str = "clear"):
    ...
    if weather not in valid:
        raise HTTPException(
            status_code=400,
            detail=f"weather must be one of {sorted(valid)}",
        )
