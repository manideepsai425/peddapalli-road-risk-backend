...
Builds the risk-weighted road graph and computes 3 route variants:
    ...
    balanced → 50/50 blend of risk + normalised time
"""
...
from model_loader import ROAD_RISK_MAP, REAL_HOTSPOTS, ModelLoader
from schemas import (
    HeatmapSegment,
    RouteCoord,
    RouteResponse,
...
class RoadRouter:
    ...
    def compute_routes(
        ...
        _valid = {"clear", "rainy", "foggy", "cloudy"}
        if weather_condition and weather_condition in _valid:
            w_info = {
        ...
        risk_cut   = (
            round((1 - best.avg_risk_score / worst_risk) * 100)
            if worst_risk > 0
            else 0
        )
        recommendation = (
            f"Take the {best.label} — {risk_cut}% safer than the riskiest option. "
            f"{best.distance_km} km | {best.duration_min} min | "
            f"{_risk_label(best.avg_risk_score)}"
        )
    ...
    def _build_network(self, W: str, IR: int, IF: int, IC: int):
        # FIX: was unindented (class-scope), causing IndentationError at import.
        loader = self.loader
        ws     = {"foggy": 4, "rainy": 3, "cloudy": 2, "clear": 1}.get(W, 1)

        MANDAL_BASE_RISK: Dict[str, float] = {
        ...
        for seg, info in loader.SEGMENT_DATA.items():
            h   = loader._hist_cnt_map.get(seg, 0)
            m   = loader._mandal_sev.get(info["mandal"]) or MANDAL_BASE_RISK.get(info["mandal"], 0.35)
            rt  = info["road_type"]
