...
FIX: predict_risk() now uses LabelEncoder with the same alphabetical
     fit order as _engineer_features(), eliminating the always-1.000
     risk score bug caused by ROAD_TYPE_ENC / WEATHER_ENC mismatch.
"""
...
# ── Encoding dicts (used only in router.py for weight calc) ──
ROAD_RISK_MAP = {"highway": 3, "junction": 3, "rural road": 2, "urban road": 1}

# NOTE: ROAD_TYPE_ENC and WEATHER_ENC are intentionally NOT used in
# predict_risk() anymore — LabelEncoder handles encoding there.
ROAD_TYPE_ENC = {"highway": 3, "junction": 2, "rural road": 1, "urban road": 0}  # kept for compat
WEATHER_ENC   = {"foggy": 3, "rainy": 2, "cloudy": 1, "clear": 0}               # kept for compat
...
# ── LabelEncoder fits — must match _engineer_features alphabetical order ──
# sklearn LabelEncoder always sorts classes alphabetically on fit_transform.
# These are pre-fit here once and reused in predict_risk() to stay in sync.
_LE_ROAD_TYPE = LabelEncoder().fit(["highway", "junction", "rural road", "urban road"])
# highway=0, junction=1, rural road=2, urban road=3

_LE_WEATHER = LabelEncoder().fit(["clear", "cloudy", "foggy", "rainy"])
# clear=0, cloudy=1, foggy=2, rainy=3

_LE_CAUSE = LabelEncoder().fit(["junction", "other", "road_design", "speed"])
# junction=0, other=1, road_design=2, speed=3
...
2023,Peddapalli,Godavari Road Peddapalli,18.6220,79.3870,urban road,1,0,0,bike,collision,clear lane violation
2023,Ramagundam,Ramagundam Railway Station,18.7589,79.4456,urban road,1,0,1,bike-auto,collision,clear poor lighting
2023,Kamanpur,Kaman Centre Town,18.6655,79.5880,urban road,1,0,0,bike,collision,clear lane violation
2024,Kamanpur,Kamanpur Cross Road,18.6712,79.5955,junction,1,0,1,bike-auto,collision,clear junction design issue
2024,Dharmaram,Mallapur Curve,18.6720,79.2310,rural road,1,0,0,bike,skid,cloudy poor road condition
2024,Sultanabad,Kodurupaka Curve,18.5850,79.3780,rural road,1,0,1,bike,skid,clear poor road condition
2024,Eligaid,Sultanabad Eligaid Link,18.5120,79.3150,rural road,1,0,0,bike,skid,clear dangerous curve
2024,Julapalli,Telukunta Curve,18.5820,79.3455,rural road,1,0,1,bike,skid,clear dangerous curve
2023,Manthani,Peddapalli Manthani Hwy,18.6422,79.6122,highway,1,0,0,bike,skid,clear dangerous curve
2023,Srirampur,Srirampur X Road,18.5255,79.3142,junction,1,0,1,bike-auto,collision,clear junction design issue
"""
...
class ModelLoader:
    ...
    def __init__(self):
        ...
        self._hist_cnt_map = {}
        self._results      = {}
        self._feature_cols = []   # ← saved during train(), used in predict_risk()
    ...
    def train(self):
        ...
        # Ensure we have both classes — if not, add synthetic minority rows
        if len(set(y)) < 2:
            ...
            y_extra = np.ones(n, dtype=int) if y.mean() < 0.5 else np.zeros(n, dtype=int)
        ...
        self._feature_cols = feature_cols   # ← save for predict_risk()
        self._ready        = True
    ...
    def predict_risk(
        ...
        # ── cause_category — match _engineer_features._mc() exactly ──
        if road_type == "highway":       cause_cat_str = "speed"
        elif road_type == "junction":    cause_cat_str = "junction"
        elif road_type == "rural road":  cause_cat_str = "road_design"
        else:                            cause_cat_str = "other"

        # ── Encode categoricals using the same LabelEncoder alphabetical
        #    order that _engineer_features() produces via fit_transform().
        ...
        # Build feature array in EXACT same column order as _engineer_features()
        # feat_cols order (after drop + LE encoding):
        # year, road_type, vehicles, weather_condition, cause_category,
        # road_type_risk, historical_accident_count, mandal_risk_level,
        # is_rainy, is_foggy, is_cloudy, multi_vehicle, heavy_vehicle_proxy,
        # year_norm, accident_intensity, weather_severity,
        # road_weather_interaction, vehicle_severity, is_junction, is_highway
        row = np.array([[
            year,
            rt_enc,
            vehicles,
    ...
    def _engineer_features(self, df_raw: pd.DataFrame):
        ...
        def _exc(s):
            s = str(s).lower()
            for c in [
                "overspeeding", "poor lighting", "poor road condition",
        ...
        # LabelEncode categoricals — sklearn always uses alphabetical order
