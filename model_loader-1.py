...
Adapted for FastAPI. One instance per process.

Key fixes vs original:
  1. _engineer_features() now reads the 'weather' column directly instead of
     parsing weather out of the 'cause' string — they always match in the CSV
     but using the dedicated column is correct and future-proof.
  2. Feature column order in predict_risk() is corrected to match the actual
     order produced by _engineer_features() after the drop + LabelEncode step.
     Original had a stale hardcoded order that diverged from the real order,
     causing silently wrong predictions.
  3. cause_reason extraction strips the weather prefix from cause before
     matching, avoiding false-positive matches on the weather word itself.
  4. _load_data() resolves path relative to __file__ with a fallback to
     looking in the CWD so it works whether run via uvicorn or pytest.
  5. weather_severity=0 is now handled correctly (falsy 0 was being overridden
     by the `or` fallback — fixed with an explicit None check).
"""
...
# ── Public constants (imported by router.py) ─────────────────────────────────
...
# ── Module-level LabelEncoders (pre-fit to known classes) ────────────────────
# sklearn LabelEncoder sorts alphabetically on fit.
# _LE_ROAD_TYPE : highway=0  junction=1  rural road=2  urban road=3
# _LE_WEATHER   : clear=0    cloudy=1    foggy=2       rainy=3
# _LE_CAUSE     : junction=0 other=1     road_design=2 speed=3
_LE_ROAD_TYPE = LabelEncoder().fit(["highway", "junction", "rural road", "urban road"])
_LE_WEATHER   = LabelEncoder().fit(["clear", "cloudy", "foggy", "rainy"])
_LE_CAUSE     = LabelEncoder().fit(["junction", "other", "road_design", "speed"])
...
_WSEV_MAP   = {"foggy": 4, "rainy": 3, "cloudy": 2, "clear": 1}
...
# ── Fallback CSV (used when data/accidents.csv is absent) ────────────────────
_STUB_CSV = """\
year,mandal,accident_prone_area,latitude,longitude,road_type,vehicles,fatalities,injuries,accident_type,weather,cause
2020,Peddapalli,Peddapalli Bus Stand,18.6155,79.3744,junction,2,1,2,bike-auto,clear,clear junction design issue
2020,Peddapalli,Andugulapalli Junction,18.6360,79.3920,junction,3,1,4,truck-car,clear,clear overspeeding
2020,Ramagundam,Godavarikhani Chowrastha,18.7502,79.4821,junction,2,1,2,truck-car,clear,clear junction design issue
2020,Ramagundam,Rajiv Rahadari GDK,18.7410,79.4580,highway,3,2,3,truck-car,clear,clear overspeeding
2020,Sultanabad,Katnapalli Gate,18.5610,79.3520,highway,3,1,4,truck-car,foggy,foggy poor visibility
2020,Sultanabad,Shastrynagar Sultanabad,18.5490,79.3350,highway,2,1,2,head-on,clear,clear overspeeding
2020,Palakurthy,Basanthnagar Bridge,18.6810,79.4525,highway,2,2,1,head-on,clear,clear road engineering defect
2020,Manthani,Godavari Bridge Manthani,18.6625,79.6633,highway,2,1,2,car-lorry,foggy,foggy road engineering defect
2020,Ramagiri,Kalvacharla Curve,18.6310,79.5410,highway,2,2,1,head-on,clear,clear overspeeding
2021,Peddapalli,Peddapalli Bus Stand,18.6155,79.3744,junction,2,0,2,bike-auto,clear,clear junction design issue
2021,Peddapalli,Andugulapalli Junction,18.6360,79.3920,junction,2,1,2,car-lorry,clear,clear overspeeding
2021,Ramagundam,Godavarikhani Chowrastha,18.7502,79.4821,junction,3,1,3,truck-auto,clear,clear junction design issue
2021,Ramagundam,Rajiv Rahadari GDK,18.7410,79.4580,highway,2,1,2,head-on,clear,clear overspeeding
2021,Sultanabad,Katnapalli Gate,18.5610,79.3520,highway,2,1,2,bike-lorry,foggy,foggy poor visibility
2021,Sultanabad,Shastrynagar Sultanabad,18.5490,79.3350,highway,3,1,3,truck-car,clear,clear overspeeding
2021,Palakurthy,Basanthnagar Bridge,18.6810,79.4525,highway,2,2,1,head-on,clear,clear road engineering defect
2021,Manthani,Godavari Bridge Manthani,18.6625,79.6633,highway,2,1,2,car-lorry,clear,clear overspeeding
2021,Ramagiri,Kalvacharla Curve,18.6310,79.5410,highway,3,2,4,truck-car,clear,clear overspeeding
2022,Peddapalli,Peddapalli Bus Stand,18.6155,79.3744,junction,3,1,2,truck-car,clear,clear junction design issue
2022,Ramagundam,Godavarikhani Chowrastha,18.7502,79.4821,junction,2,0,4,auto-bike,clear,clear junction design issue
2022,Sultanabad,Katnapalli Gate,18.5610,79.3520,highway,2,2,1,head-on,clear,clear overspeeding
2022,Palakurthy,Basanthnagar Bridge,18.6810,79.4525,highway,2,1,3,head-on,clear,clear road engineering defect
2022,Manthani,Godavari Bridge Manthani,18.6625,79.6633,highway,1,1,0,truck-skid,foggy,foggy poor visibility
2023,Peddapalli,Andugulapalli Junction,18.6360,79.3920,junction,2,1,2,car-lorry,clear,clear overspeeding
2023,Ramagundam,Rajiv Rahadari GDK,18.7410,79.4580,highway,3,2,3,truck-car,clear,clear overspeeding
2023,Sultanabad,Shastrynagar Sultanabad,18.5490,79.3350,highway,2,1,2,car-lorry,clear,clear overspeeding
2024,Peddapalli,Peddapalli Bus Stand,18.6155,79.3744,junction,2,1,3,lorry-car,cloudy,cloudy junction design issue
2024,Ramagundam,Godavarikhani Chowrastha,18.7502,79.4821,junction,1,1,0,pedestrian,clear,clear poor lighting
2024,Sultanabad,Katnapalli Gate,18.5610,79.3520,highway,3,1,3,truck-car,clear,clear overspeeding
2024,Palakurthy,Basanthnagar Bridge,18.6810,79.4525,highway,2,2,1,head-on,foggy,foggy road engineering defect
2025,Peddapalli,Andugulapalli Junction,18.6360,79.3920,junction,2,1,2,car-lorry,cloudy,cloudy junction design issue
2025,Ramagundam,Rajiv Rahadari GDK,18.7410,79.4580,highway,2,1,1,rear-end,clear,clear overspeeding
2025,Sultanabad,Shastrynagar Sultanabad,18.5490,79.3350,highway,2,2,1,car-lorry,clear,clear overspeeding
2025,Manthani,Godavari Bridge Manthani,18.6625,79.6633,highway,2,1,2,car-lorry,clear,clear overspeeding
2023,Peddapalli,Godavari Road Peddapalli,18.6220,79.3870,urban road,1,0,0,bike,clear,clear lane violation
2023,Ramagundam,Ramagundam Railway Station,18.7589,79.4456,urban road,1,0,1,bike-auto,clear,clear poor lighting
2023,Kamanpur,Kaman Centre Town,18.6655,79.5880,urban road,1,0,0,bike,clear,clear lane violation
2024,Kamanpur,Kamanpur Cross Road,18.6712,79.5955,junction,1,0,1,bike-auto,clear,clear junction design issue
2024,Dharmaram,Mallapur Curve,18.6720,79.2310,rural road,1,0,0,bike,cloudy,cloudy poor road condition
2024,Sultanabad,Kodurupaka Curve,18.5850,79.3780,rural road,1,0,1,bike,clear,clear poor road condition
2024,Eligaid,Sultanabad Eligaid Link,18.5120,79.3150,rural road,1,0,0,bike,clear,clear dangerous curve
2024,Julapalli,Telukunta Curve,18.5820,79.3455,rural road,1,0,1,bike,clear,clear dangerous curve
2023,Manthani,Peddapalli Manthani Hwy,18.6422,79.6122,highway,1,0,0,bike,clear,clear dangerous curve
2023,Srirampur,Srirampur X Road,18.5255,79.3142,junction,1,0,1,bike-auto,clear,clear junction design issue
"""
...
class ModelLoader:
    ...
    # ── Feature column order produced by _engineer_features() ────────────────
    # Kept as a class constant so predict_risk() can assert alignment at runtime.
    _EXPECTED_FEAT_ORDER = [
        "year", "road_type", "vehicles", "weather_condition",
        "accident_intensity", "weather_severity", "road_type_risk",
        "road_weather_interaction", "vehicle_severity", "cause_category",
        "historical_accident_count", "mandal_risk_level",
        "is_rainy", "is_foggy", "is_cloudy",
        "multi_vehicle", "heavy_vehicle_proxy", "year_norm",
    ...
    def __init__(self) -> None:
        ...
        self._mandal_sev:  dict = {}
        self._hist_cnt_map: dict = {}
        self._results:     dict = {}
        self._feature_cols: list = []

        # Pre-compute edge list (geometry only — no model needed)
        self.EDGE_LIST: list = []
    ...
    def resolve_location(self, name: str):
        """Return canonical segment name or None."""
    ...
    def train(self) -> None:
        ...
        # Sanity-check feature order matches the expected constant
        if feature_cols != self._EXPECTED_FEAT_ORDER:
            log.warning(
                "Feature column order mismatch!\n"
                f"  Got:      {feature_cols}\n"
                f"  Expected: {self._EXPECTED_FEAT_ORDER}"
            )

        X = df_ml[feature_cols].values
        y = df_ml["high_risk"].values

        # Guard: ensure both classes present
        if len(set(y)) < 2:
            log.warning("⚠ Only one class in training data — adding synthetic minority rows")
            n = max(5, len(y) // 10)
            X_extra = np.tile(X.mean(axis=0), (n, 1))
            X_extra += np.random.normal(0, 0.1, X_extra.shape)
            y_extra = (np.ones(n, dtype=int) if y.mean() < 0.5
        ...
        # SMOTE only when class ratio is imbalanced
        ...
        # Severity and history maps derived from the full dataset
        df["severity_score"] = (
            (df["fatalities"] * 3 + df["injuries"])
            / max(df["fatalities"].max() * 3 + df["injuries"].max(), 1)
        )
        ...
        log.info(
            "✅ Best model: %s | Accuracy=%.4f | F1=%.4f | AUC=%.4f | Features=%d",
            best_name,
    ...
    def predict_risk(
        ...
        vehicles: int = 2,
        year_norm: float = 1.0,
        accident_intensity: float = 0.3,
        weather_severity: int = None,
        is_junction: int = 0,
        is_highway: int = 0,
        segment_name: str = "",
    ) -> float:
        # FIX: use explicit None check — weather_severity=0 must not fall
        # through to the dict lookup (0 is falsy but is a valid value).
        ws = _WSEV_MAP.get(weather_cond, 1) if weather_severity is None else weather_severity
        rwi  = road_type_risk * ws
        vsev = vehicles * accident_intensity

        # cause_category mirrors _engineer_features._mc() logic
        if road_type == "highway":
            cause_cat_str = "speed"
        elif road_type == "junction":
            cause_cat_str = "junction"
        elif road_type == "rural road":
            cause_cat_str = "road_design"
        else:
            cause_cat_str = "other"

        rt_enc = int(_LE_ROAD_TYPE.transform([road_type])[0])
        wc_enc = int(_LE_WEATHER.transform([weather_cond])[0])
        cc_enc = int(_LE_CAUSE.transform([cause_cat_str])[0])

        # Feature order MUST match _EXPECTED_FEAT_ORDER / _engineer_features():
        # year, road_type, vehicles, weather_condition,
        # accident_intensity, weather_severity, road_type_risk,
        # road_weather_interaction, vehicle_severity, cause_category,
        # historical_accident_count, mandal_risk_level,
        ...
        # Trim/pad defensively in case feature count shifts
        n_feat = len(self._feature_cols)
        if row.shape[1] > n_feat:
            row = row[:, :n_feat]
        elif row.shape[1] < n_feat:
            row = np.hstack([row, np.zeros((1, n_feat - row.shape[1]))])

        prob  = float(self._model.predict_proba(self._scaler.transform(row))[0][1])
        mult  = {"rainy": 1.18, "foggy": 1.22, "cloudy": 1.08}.get(weather_cond, 1.0)
        score = float(np.clip(prob * mult, 0.0, 1.0))

        # Confirmed fatal hotspots always score ≥ 0.72
    ...
    def get_current_weather(self, time_pref: str = "now") -> dict:
        ...
        try:
            resp = requests.get(
                ...
                params={
                    "latitude": 18.61, "longitude": 79.37,
            ...
            cond = (
                "rainy"  if code in [51, 53, 55, 61, 63, 65, 80, 81, 82]
        ...
        # Time-of-day override: late night / early morning → foggy
    ...
    def _load_data(self) -> pd.DataFrame:
        # Try <script_dir>/data/accidents.csv, then CWD/data/accidents.csv
        for base in (os.path.dirname(os.path.abspath(__file__)), os.getcwd()):
            p = os.path.join(base, "data", "accidents.csv")
            if os.path.exists(p):
                log.info("Loading accident data from %s", p)
    ...
    def _engineer_features(self, df_raw: pd.DataFrame):
        ...
        # ── Weather: read directly from the 'weather' column ─────────────────
        # FIX: original code extracted weather by searching the 'cause' string.
        # The CSV has a dedicated 'weather' column — use it directly.
        def _norm_weather(s: str) -> str:
            s = str(s).lower().strip()
        ...
        # ── Cause reason: strip the leading weather word before matching ──────
        # cause format in CSV: "<weather> <cause_reason>", e.g. "clear overspeeding"
        def _exc(s: str) -> str:
            s = str(s).lower().strip()
            # Remove leading weather token so it doesn't pollute cause matching
            parts = s.split(" ", 1)
            body  = parts[1] if len(parts) > 1 else s
            for c in (
                "overspeeding", "wet road surface", "poor visibility",
                "poor lighting", "poor road condition", "junction design issue",
                "dangerous curve", "lack of signage", "road engineering defect",
                "lane violation",
            ):
                if c in body:
                    return c
        ...
        def _mc(c: str) -> str:
            c = str(c).lower()
            if "speed" in c or "overspeeding" in c:
                return "speed"
            if "junction" in c:
                return "junction"
            if "road" in c or "curve" in c or "engineering" in c or "condition" in c:
                return "road_design"
            return "other"

        df["weather_condition"] = df["weather"].apply(_norm_weather)
        df["cause_reason"]      = df["cause"].apply(_exc)

        # Target variable
        df["high_risk"] = (
            (df["fatalities"] > 0) | (df["injuries"] >= 2)
        ).astype(int)

        mf = max(df["fatalities"].max(), 1)
        mi = max(df["injuries"].max(), 1)
        df["accident_intensity"] = (df["fatalities"] / mf) * 0.6 + (df["injuries"] / mi) * 0.4

        df["weather_severity"]         = df["weather_condition"].map(_WSEV_MAP).fillna(1)
        ...
        df["heavy_vehicle_proxy"] = df["accident_type"].str.contains(
            "truck|lorry|bus|tipper|dumper", case=False, na=False
        ).astype(int)
        df["year_norm"]   = (
            (df["year"] - df["year"].min())
            / max(1, df["year"].max() - df["year"].min())
        )
        df["is_junction"] = (df["road_type"] == "junction").astype(int)
        df["is_highway"]  = (df["road_type"] == "highway").astype(int)

        drop_cols = [
            "cause", "cause_reason", "accident_prone_area", "mandal",
            "accident_type", "weather",          # 'weather' replaces 'accident_movement'
            "latitude", "longitude",
            "fatalities", "injuries", "severity_score",
        ]
        df_ml = df.drop(columns=drop_cols, errors="ignore")

        # LabelEncode in alphabetical order (sklearn default)
